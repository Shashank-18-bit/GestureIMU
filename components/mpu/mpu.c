...
esp_err_t enable_mpu(void)
{
    ...
    // Set Clock Source
    uint8_t value;
    I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_read_reg(&mpu_dev_t, mpu_RA_PWR_MGMT_1, &value, 1));
    value = value & 0xf8;
    value = value | mpu_CLOCK_PLL_XGYRO;
    I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_write_reg(&mpu_dev_t, mpu_RA_PWR_MGMT_1, &value, 1));

    // Set fullscale gyro range
    value = mpu_GYRO_FS_250;
    I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_write_reg(&mpu_dev_t, mpu_RA_GYRO_CONFIG, &value, 1));

    // Set fullscale accel range
    value = mpu_ACCEL_FS_2;
    I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_write_reg(&mpu_dev_t, mpu_RA_ACCEL_CONFIG, &value, 1));
...
esp_err_t mpu_read_acce(uint8_t *data_rd, size_t size)
{
    ...
    I2C_DEV_TAKE_MUTEX(&mpu_dev_t);
    I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_read_reg(&mpu_dev_t, ACCE_START_ADDR, data_rd, size));
    I2C_DEV_GIVE_MUTEX(&mpu_dev_t);
...
esp_err_t mpu_read_gyro(uint8_t *data_rd, size_t size)
{
    ...
    I2C_DEV_TAKE_MUTEX(&mpu_dev_t);
    I2C_DEV_CHECK(&mpu_dev_t, i2c_dev_read_reg(&mpu_dev_t, GYRO_START_ADDR, data_rd, size));
    I2C_DEV_GIVE_MUTEX(&mpu_dev_t);
...
esp_err_t read_mpu_raw(int16_t *acce_raw_value, int16_t *gyro_raw_value)
{
    ...
    if (mpu_read_acce(acce_rd, BUFF_SIZE) == ESP_OK && mpu_read_gyro(gyro_rd, BUFF_SIZE) == ESP_OK)
        err = ESP_OK;
